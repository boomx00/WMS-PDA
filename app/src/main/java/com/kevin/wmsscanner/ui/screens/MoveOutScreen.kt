...
@Composable
fun MoveOutScreen(navController: NavHostController) {
    ...
    Column(
        ...
        Text("Move Out", style = MaterialTheme.typography.headlineMedium)
        Text(
            "Rack \u2192 Floor",
        ...
        OutlinedTextField(
            ...
            label = { Text("Scan current rack location") },
            singleLine = true,
            modifier = Modifier
                .fillMaxWidth()
        ...
        Button(
                    ...
                    try {
                        val response = NetworkModule.api.movePallet(
                            MoveRequest(
                                ...
                                newLocationCode = "FLOOR",
                                quantity = quantityInput.toIntOrNull()
                            )
                        )
                        loading = false
                        if (response.isSuccessful) {
                            success = "Moved ${labelInput.trim()} to Floor"
                            labelInput = ""
            ...
            enabled = !loading && labelInput.isNotBlank() && currentLocationInput.isNotBlank(),
            modifier = Modifier.fillMaxWidth().height(56.dp)
        ) {
            Text(if (loading) "Moving..." else "Confirm Move Out")
        }
