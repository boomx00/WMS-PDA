...
@Composable
fun MoveInScreen(navController: NavHostController) {
    ...
    Column(
        ...
        Text("Move In", style = MaterialTheme.typography.headlineMedium)
        Text(
            "Floor \u2192 Rack",
                    ...
                    try {
                        val response = NetworkModule.api.movePallet(
                            MoveRequest(
                                ...
                                currentLocationCode = "FLOOR",
                                newLocationCode = locationInput.trim(),
                                quantity = quantityInput.toIntOrNull()
                            )
                        )
                        loading = false
                        if (response.isSuccessful) {
                            success = "Moved ${labelInput.trim()} to ${locationInput.trim()}"
                            labelInput = ""
                            locationInput = ""
                            quantityInput = ""
                            needsQuantity = false
